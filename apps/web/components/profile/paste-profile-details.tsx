...
export function PasteProfileDetails({ onApply }: { onApply: (data: Partial<PassportData>) => void }) {
  ...
  return <Dialog open={open} onOpenChange={next => { setOpen(next); if (!next) setText(""); }}>
    ...
    <DialogContent className="app-modal"><DialogHeader className="app-modal-header is-stacked"><DialogTitle>Вставка реквизитов</DialogTitle><DialogDescription>Скопируйте доступные вам данные и вставьте сюда. Вставленный текст не подтверждает личность или подлинность данных MAX.</DialogDescription></DialogHeader>
