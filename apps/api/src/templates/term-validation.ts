...
export function hasPaymentTime(text: string): boolean {
  ...
  return new RegExp(`(?:после|перед|до|при|в день|по факту|по|с момента)\\s+(?:[а-я]+\\s+){0,2}${EVENT}`, "u").test(text)
...
export function paymentIssue(text: string, direct = false): string | null {
  ...
  if (!hasPaymentTime(withoutNegation)) return "Не удалось определить срок оплаты. Укажите дату или событие: «В день приёмки» либо «В течение 3 дней после подписания акта». Способ перевода или отсутствие аванса сами по себе не задают срок.";
