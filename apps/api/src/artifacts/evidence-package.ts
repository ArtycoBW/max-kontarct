...
export async function buildEvidencePackage(input: EvidencePackageInput): Promise<Buffer> {
  ...
  add({
    body: Buffer.from([
      "Технический пакет материалов сделки «Макс-Контракт».",
      "",
      "Пакет содержит итоговый PDF, разрешённые участникам приложения, релевантный аудит, записи ПЭП и контрольные суммы.",
      "Он помогает проверить техническую целостность материалов, но не является гарантией принятия конкретным судом или государственным органом.",
    ].join("\n"), "utf8"),
